...
func (app *AppState) downloadMultipleFiles(filePath, outputFile, limit, directory string) error {
	...
	for scanner.Scan() {
		...
		if url == "" {
			continue // Skip emptyquochieng@LAP-034:~/Module/wget-gitea$ git commit -m "" lines
		}
		wg.Add(1)
...
func (app *AppState) AsyncDownload(outputFileName, url, limit, directory string) error {
	...
	resp, err := utils.HttpRequest(url)func (app *AppState) AsyncDownload(outputFileName, url, limit, directory string) error {
		path, err := utils.ExpandPath(directory)
		if err != nil {
			return err
		}
